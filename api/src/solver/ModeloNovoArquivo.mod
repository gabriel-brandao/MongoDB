...
{tripla} IRS = {<i,r,s>|r in R, s in 1..M[r], i in Ir[r]}; //Tuplas com o conjunto de categorias da inst�ncia e conjunto de m�dulos poss�veis para cada categoria
		...
		if(thisOplModel.z[m]>0){
			...
			for(var k in K){
				...
				if(tiposItens>0){
					...
					if(contaIter >= 1000){
						ofile.write("Erro de ordenacao:Limite de iteracoes atingido");					
					}
					if(testeValidade == 0){
