...
execute{
	...
	writeln("no_modulos ",contaModulos);
	writeln();
	var aux = 1;
	for(var m in RS){
		if(thisOplModel.z[m]>0){
			...
			writeln("no_niveis ",contaNiveis);
			writeln();
			var contNivel = 1;
			for(var k in K){
				writeln("nivel ",contNivel);
				contNivel++;		
				writeln("altura ",thisOplModel.y[k]);
				var tiposItens = 0;
				for(var irs in IRS){
					if(m.i==irs.r && m.j==irs.s && thisOplModel.alfa[irs.i][k][m]>0 ){
						tiposItens++;				
					}			
				}
				writeln("tipos_itens ",tiposItens);
				if(tiposItens>0){
					var indiceItem;
					var vetItens = new Array();
					var vetQtde = new Array();
					for(irs in IRS){
						if(irs.r == m.i && irs.s == m.j && thisOplModel.deltaI[irs.i][k][m]==1){
							indiceItem = irs.i;
							vetItens[1] = indiceItem;
							vetQtde[1] = thisOplModel.alfa[irs.i][k][m];		
    					}						
					}
					var indiceVets = 1;
					var indiceFinal;
					for(irs in IRS){
						if(irs.r == m.i && irs.s == m.j && thisOplModel.deltaF[irs.i][k][m]==1){
							indiceFinal = irs.i;
    					}						
					}
					var testeValidade = 1;
					...
					while(indiceItem!=indiceFinal && testeValidade == 1 && contaIter<1000){
						...
						for(var j in Ir[m.i]){
							if(indiceItem!=j){
								if(thisOplModel.beta[k][B.find(indiceItem,j)][m]==1){
									indiceVets++;
									indiceItem = j;
									vetItens[indiceVets] = j;
									vetQtde[indiceVets] = thisOplModel.alfa[j][k][m];
									testeValidade = 1;
									break;					
								}
      						}													
					...
					if(contaIter >= 1000){
						write("Erro de ordenação:Limite de iterações atingido");					
					}
					if(testeValidade == 0){
						write("Erro de continuidade!");					
					}
					write("itens ");
